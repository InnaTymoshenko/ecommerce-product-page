...
const CartComponent = ({ setIsCart }: CartProps) => {
	...
	return (
		<div className="absolute top-0 left-0 h-full w-full bg-transparent" onClick={() => setIsCart(false)}>
						...
						<div className="w-full flex justify-between items-center gap-4 ">
							...
							<div className="w-[60%] flex flex-col justify-between items-start gap-2">
								...
								<div className="flex gap-2 items-center justify-start">
									...
									{product.count && (
										<strong className="text-blue-500">{`$${(125.0 * product.count).toFixed(2)}`}</strong>
									)}
								</div>
							</div>
